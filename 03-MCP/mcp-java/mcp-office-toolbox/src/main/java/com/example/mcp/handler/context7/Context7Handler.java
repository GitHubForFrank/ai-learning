package com.example.mcp.handler.context7;
 ...
 */
@Service
public class Context7Handler extends BaseHandler {
    ...
    public Context7Handler(@Value("${fetch.timeout-seconds}") int timeoutSeconds) {
     ...
     */
    @Tool(name = "context7_resolve_library_id", description = "将库名称解析为 Context7 标准库 ID。输入库名称，返回匹配的库 ID 列表及元数据。")
    public String resolveLibraryId(@ToolParam(description = "请求参数，包含 query（用户问题）和 libraryName（库名称）") ResolveLibraryRequest request) {
        return execute("resolveLibraryId", () -> {
     ...
     * @return 文档片段和代码示例
     */
    @Tool(name = "context7_query_docs", description = "根据库 ID 查询最新文档。返回指定库的代码示例和文档片段。")
        ...
        return execute("queryDocs", () -> {
            ...
            ResponseEntity<Map> response = restClient.get()
                                                     ...
                                                     .onStatus(status -> status.is4xxClientError(), (req, res) -> {
                                                         ...
                                                         throw new IllegalArgumentException("资源不存在或请求错误: " + errorBody);
                                                     })
                                                     .toEntity(Map.class);
