...
public class Context7Handler {
    ...
    public String resolveLibraryId(
            @ToolParam(description = "请求参数，包含 query（用户问题）和 libraryName（库名称）") ResolveLibraryRequest request) {
        try {
            log.info("resolve-library-id: libraryName={}, query={}", request.libraryName(), request.query());

            ResponseEntity<Map> response = restClient.get()
                    .uri(URI.create(API_BASE_URL + "/v2/libs/search?libraryName=" +
                            encode(request.libraryName()) + "&query=" + encode(request.query())))
                    .retrieve()
                    .toEntity(Map.class);
            ...
            sb.append("找到 ").append(results.size()).append(" 个匹配的库：\n\n");
            ...
            for (int i = 0; i < Math.min(results.size(), 5); i++) {
                ...
                if (totalSnippets != null) {
                    sb.append("   代码片段数: ").append(totalSnippets).append("\n");
                }
                if (trustScore != null) {
                    sb.append("   可信度评分: ").append(String.format("%.1f/10", trustScore.doubleValue())).append("\n");
                }
                if (versions != null && !versions.isEmpty()) {
                    sb.append("   可用版本: ").append(String.join(", ", versions.subList(0, Math.min(versions.size(), 5))));
                    if (versions.size() > 5) {
                        sb.append(" 等").append(versions.size()).append("个版本");
                    }
                    sb.append("\n");
                }
                sb.append("\n");
    ...
    public String queryDocs(
        ...
        try {
            log.info("query-docs: libraryId={}, query={}, tokens={}",
                    request.libraryId(), request.query(), request.tokens());

            String url = API_BASE_URL + "/v2/context?libraryId=" + encode(request.libraryId())
            ...
            // 解析并格式化文档内容
            StringBuilder sb = new StringBuilder();
            sb.append("库 ID: ").append(request.libraryId()).append("\n");
            sb.append("查询: ").append(request.query()).append("\n\n");
