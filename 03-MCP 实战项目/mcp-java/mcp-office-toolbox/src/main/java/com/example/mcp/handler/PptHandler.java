...
public class PptHandler {
    ...
    public String createPpt(
            @ToolParam(description = "Absolute path for the new PPT file") String fileAbsolutePath) {
        try {
    ...
    public String addPptSlide(
            @ToolParam(description = "Absolute path to the PPT file") String fileAbsolutePath,
        ...
        try {
            ...
            try (XMLSlideShow slideShow = openSlideShow(path)) {
                ...
                saveSlideShow(slideShow, path);
                return "已添加第 " + slideShow.getSlides().size() + " 张幻灯片: " + fileAbsolutePath;
            }
        } catch (Exception e) {
    ...
    public String readPptText(
        ...
        try {
            ...
            try (XMLSlideShow slideShow = openSlideShow(path)) {
                ...
                List<XSLFSlide> slides = slideShow.getSlides();
                sb.append("PPT 文件: ").append(fileAbsolutePath).append("\n");
                sb.append("总幻灯片数: ").append(slides.size()).append("\n\n");
    ...
    public String modifyPptSlideText(
            @ToolParam(description = "Absolute path to the PPT file") String fileAbsolutePath,
            @ToolParam(description = "Slide index to modify (0-based)") int slideIndex,
    ...
    public String savePptAs(
            @ToolParam(description = "Absolute path to the source PPT file") String sourcePath,
