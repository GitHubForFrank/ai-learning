...
public class ZipHandler {
    ...
    public String zipCompress(
            @ToolParam(description = "源文件或目录路径，多个以逗号分隔") String sourcePaths,
    ...
    public String zipDecompress(
            @ToolParam(description = "ZIP 文件路径") String zipFilePath,
    ...
    public String zipList(
        ...
        try {
            ...
            sb.append("ZIP 文件内容列表: ").append(zipFilePath).append("\n");
            ...
            for (ZipEntryInfo entry : entries) {
                ...
                sb.append(String.format("%-50s %-10s %-10s %s\n",
                        entry.name().length() > 48 ? entry.name().substring(0, 45) + "..." : entry.name(),
            ...
            sb.append("\n");
            sb.append("原始总大小: ").append(formatSize(totalSize)).append("\n");
            sb.append("压缩后总大小: ").append(formatSize(totalCompressed)).append("\n");
            if (totalSize > 0) {
    ...
    public String zipCompressDirectory(
            @ToolParam(description = "要压缩的目录路径") String dirPath,
        ...
        try {
            ...
            if (zipOutputPath != null && !zipOutputPath.isBlank()) {
                ...
                outputPath = dir.getParent().resolve(dirName + ".zip");
            }
