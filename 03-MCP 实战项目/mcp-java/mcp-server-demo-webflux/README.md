# mcp-server-demo-webflux

> 基于 Spring AI MCP Server Starter 的天气查询 MCP Server，**WebFlux（响应式）HTTP + SSE** 传输版本。数据源为美国国家气象局（weather.gov）。
...
```bash
java -jar target/mcp-server-demo-webflux-*.jar
```

**STDIO 模式**（同一个 jar 切到 stdio，适合本地工具）：

```bash
java -Dspring.ai.mcp.handler.stdio=true \
     -Dspring.main.web-application-type=none \
     -jar target/mcp-server-demo-webflux-*.jar
```
...
{
  "mcpServers": {
    "weather-webflux": { "url": "http://localhost:8080" }
  }
}
```
  ...
  "mcpServers": {
    "weather-webflux": {
      ...
      "args": [
        ...
        "-Dspring.main.web-application-type=none",
        "-jar", "/absolute/path/to/target/mcp-server-demo-webflux-0.0.1-SNAPSHOT.jar"
      ]
    }
  }
}
