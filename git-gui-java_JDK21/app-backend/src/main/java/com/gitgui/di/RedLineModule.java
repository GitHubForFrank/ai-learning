...
public class RedLineModule extends AbstractModule {
    ...
    protected void configure() {
        bind(CommandRedLineService.class).to(CommandRedLineServiceImpl.class)
                                         .in(Singleton.class);
        ...
        // 阻断类（BLOCK，BR-26）
        ruleBinder.addBinding()
                  .to(ForcePushRule.class);
        ruleBinder.addBinding()
                  .to(ProtectedBranchRule.class);
        ruleBinder.addBinding()
                  .to(DeleteProtectedBranchRule.class);
        ruleBinder.addBinding()
                  .to(SensitiveFileRule.class);
        ruleBinder.addBinding()
                  .to(RemoteWhitelistRule.class);
        ruleBinder.addBinding()
                  .to(NoVerifyRule.class);
        // 二次确认类（CONFIRM，BR-29）
        ruleBinder.addBinding()
                  .to(ResetHardRule.class);
        ruleBinder.addBinding()
                  .to(CleanFdxRule.class);
        ruleBinder.addBinding()
                  .to(AmendPushedRule.class);
        ruleBinder.addBinding()
                  .to(RebasePushedRule.class);
        ruleBinder.addBinding()
                  .to(FilterBranchRule.class);
        ruleBinder.addBinding()
                  .to(LargeFileRule.class);
        ruleBinder.addBinding()
                  .to(RedLineToggleRule.class);
    }
}
